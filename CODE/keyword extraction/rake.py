...
class Rake(object):
    ...
    def extract_keywords_from_text(self, text, graphscore):
        """Method to extract keywords from the text provided.

        :param text: Text to extract keywords from, provided as a string.
        """
        sentences = nltk.tokenize.sent_tokenize(text)
        self.extract_keywords_from_sentences(sentences, graphscore)

    def extract_keywords_from_sentences(self, sentences, graphscore):
        ...
        self._build_ranklist(phrase_list, graphscore)
    ...
    def _build_ranklist(self, phrase_list, graphscore):
        ...
        for phrase in phrase_list:
            ...
            for word in phrase:
                if self.metric == Metric.DEGREE_TO_FREQUENCY_RATIO:
                    rank += 1.0 * graphscore[word] * self.degree[word] / self.frequency_dist[word]
                elif self.metric == Metric.WORD_DEGREE:
                    rank += 1.0 * graphscore[word] * self.degree[word]
                else:
                    rank += 1.0 * graphscore[word] * self.frequency_dist[word]
    ...
    def _generate_phrases(self, sentences):
        ...
        for sentence in sentences:
            word_list = [word.lower() for word in wordpunct_tokenize(sentence) if word.isalnum()]
    ...
    def _get_phrase_list_from_words(self, word_list):
        ...
        """
        groups = groupby(word_list, lambda x: x not in self.to_ignore and x.isalnum())
